...
import recoder  from "@/data/recoder-0";
import dslSol from '@/data/solver-dsl-0';
import final_result from "@/data/final_result";
import dbscan_result from "@/data/dbscan_result";
	...
	useEffect(()=> {
		...
		const data = await curQAs.map((rData) => {
				  const result =  rData.each_recoder.map((bData) => {
				       return {id: rData.id, data: bData, dsl: dslSol["solve_"+rData.id].map(dslSingle=><p>{dslSingle}</p> )}
	//			       (<div>
